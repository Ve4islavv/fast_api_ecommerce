...
async def update_product(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    if (get_user.get('is_admin') or
        (get_user.get('is_supplier') and
            (get_user.get('id') == product_update.supplier_id or product_update.supplier_id is None))):
        await db.execute(
...
async def delete_product(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    if (get_user.get('is_admin') or
            (get_user.get('is_supplier') and
            (get_user.get('id') == product_delete.supplier_id or product_delete.supplier_id is None))):
